...
class EntropyAL(ALStrategy):
    ...
    def select_indices(self):
        probs = list(self.model.get_probabilities(self.unannotated_corpus))
        entropy = np.sum(-np.array(probs) * np.log(np.array(probs)), axis=1)
...
class CAL(ALStrategy):
    ...
    def select_indices(self):
        ...
        train_output = list(self.model.predict(self.annotated_corpus))

        ids_unlabeled = range(len(self.unannotated_corpus))
        embeds_unlabeled = list(self.model.get_embeddings(self.unannotated_corpus))
        logits_unlabeled = torch.from_numpy(np.array(train_output))

        # labeled_ids = [inst["id"] for inst in labeled_data]
        embeds_labeled = list(self.model.get_embeddings(self.annotated_corpus))
        logits_labeled = torch.from_numpy(np.array(train_output))
...
class CoreSetAL(ALStrategy):
    ...
    def select_indices(self):
        ids = range(len(self.unannotated_corpus))
        #get the penultimate layer embeddings from trainer.model
        unlabeled_embeds = list(self.model.get_embeddings(self.unannotated_corpus))
        labeled_embeds = list(self.model.get_embeddings(self.annotated_corpus))
...
class PRC(ALStrategy):
    def __init__(self, **kwargs):
        ...
        self.rare_class = kwargs['rare_class']#default is 1: for positive rare class
    
    def select_indices(self):
        probs = list(self.model.get_probabilities(self.unannotated_corpus))
        indices = np.argsort([p[self.rare_class] for p in probs],)[::-1][:self.batch_size]
        return indices
